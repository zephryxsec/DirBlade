...
def check_path(target_url, path, status_codes, timeout, output_lock, output_file):
    ...
    if response is not None and response.status_code in status_codes:
        line = f"[+] Discovered Directory -----> {test_url} ({response.status_code})"
        with output_lock:
            print(line)
...
def dir_bruteforce(target_url, wordlist_path, status_codes, timeout, threads, extensions, output_path):
    ...
    try:
        with concurrent.futures.ThreadPoolExecutor(max_workers=threads) as executor:
            futures = [
                executor.submit(check_path, target_url, path, status_codes, timeout, output_lock, output_file)
                for path in paths
            ]
...
def main():
    ...
    parser.add_argument(
        "-x", "--extensions",
        default="",
        help="Comma-separated file extensions to also try for each wordlist entry (ex. php,html,txt)",
    )
    parser.add_argument("-o", "--output", help="Path to write discovered results to, in addition to stdout")
    ...
    print("Extensions:", extensions if extensions else "none")
    print("Output file:", args.output if args.output else "none")
    print("")

    dir_bruteforce(target_url, wordlist_path, status_codes, args.timeout, args.threads, extensions, args.output)
